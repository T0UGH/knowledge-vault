---
title: reactor
source_url: https://www.xiaolincoding.com/os/8_network_system/reactor.html
fetched_at: 2026-04-06
section: os
---

[#](https://www.xiaolincoding.com#_9-3-高性能网络模式-reactor-和-proactor) 9.3 高性能网络模式：Reactor 和 Proactor
小林，来了。
...
[#](https://www.xiaolincoding.com#演进) 演进
...
当一个连接对应一个线程时，线程一般采用「read -> 业务处理 -> send」的处理流程，如果当前连接没有数据可读，那么线程会阻塞在 `read`
操作上（ socket 默认情况是阻塞 I/O），不过这种阻塞方式并不影响其他线程。

但是引入了线程池，那么一个线程要处理多个连接的业务，线程在处理某个连接的 `read`
操作时，如果遇到没有数据可读，就会发生阻塞，那么线程就没办法继续处理其他连接的业务。

要解决这一个问题，最简单的方式就是将 socket 改成非阻塞，然后线程不断地轮询调用 `read`
操作来判断是否有数据，这种方式虽然该能够解决阻塞的问题，但是解决的方式比较粗暴，因为轮询是要消耗 CPU 的，而且随着一个 线程处理的连接越多，轮询的效率就会越低。

上面的问题在于，线程并不知道当前连接是否有数据可读，从而需要每次通过 `read`
去试探。
...
PS：如果想知道 select/poll/epoll 的区别，可以看看小林之前写的这篇文章：

[这次答应我，一举拿下 I/O 多路复用！ (opens new window)]
...
[#](https://www.xiaolincoding.com#reactor) Reactor
[#](https://www.xiaolincoding.com#单-reactor-单进程-线程) 单 Reactor 单进程 / 线程
...
[#](https://www.xiaolincoding.com#单-reactor-多线程-多进程) 单 Reactor 多线程 / 多进程
...
[#](https://www.xiaolincoding.com#多-reactor-多进程-线程) 多 Reactor 多进程 / 线程
...
[#](https://www.xiaolincoding.com#proactor) Proactor
