---
title: define TASK_SIZE __PAGE_OFFSET
source_url: https://www.xiaolincoding.com/os/3_memory/linux_mem.html
fetched_at: 2026-04-06
section: os
---

[#](https://www.xiaolincoding.com#_4-6-深入理解-linux-虚拟内存管理) 4.6 深入理解 Linux 虚拟内存管理
作者：公众号@bin的技术小屋

来源：

[3.5 万字 + 60 张图 ｜一步一图带你深入理解 Linux 虚拟内存管理 (opens new window)]
...
[#](https://www.xiaolincoding.com#_1-到底什么是虚拟内存地址) 1. 到底什么是虚拟内存地址
...
[#](https://www.xiaolincoding.com#_2-为什么要使用虚拟地址访问内存) 2. 为什么要使用虚拟地址访问内存
...
[#](https://www.xiaolincoding.com#_3-进程虚拟内存空间) 3. 进程虚拟内存空间
...
[#](https://www.xiaolincoding.com#_4-linux-进程虚拟内存空间) 4. Linux 进程虚拟内存空间
在上小节中我们介绍了进程虚拟内存空间中各个内存区域的一个大概分布，在此基础之上，本小节我就带大家分别从 32 位 和 64 位机器上看下在 Linux 系统中进程虚拟内存空间的真实分布情况。

[#](https://www.xiaolincoding.com#_4-1-32-位机器上进程虚拟内存空间分布) 4.1 32 位机器上进程虚拟内存空间分布
...
[#](https://www.xiaolincoding.com#_4-2-64-位机器上进程虚拟内存空间分布) 4.2 64 位机器上进程虚拟内存空间分布
上小节中介绍的 32 位虚拟内存空间布局和本小节即将要介绍的 64 位虚拟内存空间布局都可以通过 `cat /proc/pid/maps`
或者 `pmap pid`
来查看某个进程的实际虚拟内存布局。
...
[#](https://www.xiaolincoding.com#_5-进程虚拟内存空间的管理) 5. 进程虚拟内存空间的管理
...
...... 省略 ......
```
由于本小节中我们举的示例是通过 fork() 函数创建子进程的情形，所以这里大家先占时忽略 `if (clone_flags & CLONE_VM)`
这个条件判断逻辑，我们先跳过往后看~~
...
而当我们通过 vfork 或者 clone 系统调用创建出的子进程，首先会设置 CLONE_VM 标识，这样来到 copy_mm 函数中就会进入 `if (clone_flags & CLONE_VM)`
条件中，在这个分支中会将父进程的虚拟内存空间以及相关页表直接赋值给子进程。这样一来父进程和子进程的虚拟内存空间就变成共享的了。也就是说父子进程之间使用的虚拟内存空间是一样的，并不是一份拷贝。
...
[#](https://www.xiaolincoding.com#_5-1-内核如何划分用户态和内核态虚拟内存空间) 5.1 内核如何划分用户态和内核态虚拟内存空间
...
我们来看下内核在 `/arch/x86/include/asm/page_32_types.h`
文件中关于 TASK_SIZE 的定义。
...
我们来看下内核在 `/arch/x86/include/asm/page_64_types.h`
文件中关于 TASK_SIZE 的定义。
...
[#](https://www.xiaolincoding.com#_5-2-内核如何布局进程虚拟内存空间) 5.2 内核如何布局进程虚拟内存空间
...
[#](https://www.xiaolincoding.com#_5-3-内核如何管理虚拟内存区域) 5.3 内核如何管理虚拟内存区域
...
[#](https://www.xiaolincoding.com#_5-4-定义虚拟内存区域的访问权限和行为规范) 5.4 定义虚拟内存区域的访问权限和行为规范
...
vm_flags 则偏向于定于整个虚拟内存区域的访问权限以及行为规范。描述的是虚拟内存区域中的整体信息，而不是虚拟内存区域中具体的某个独立页面。它是一个抽象的概念。可以通过 `vma->vm_page_prot = vm_get_page_prot(vma->vm_flags)`
...
[#](https://www.xiaolincoding.com#_5-5-关联内存映射中的映射关系) 5.5 关联内存映射中的映射关系
...
[#](https://www.xiaolincoding.com#_5-6-针对虚拟内存区域的相关操作) 5.6 针对虚拟内存区域的相关操作
...
[#](https://www.xiaolincoding.com#_5-7-虚拟内存区域在内核中是如何被组织的) 5.7 虚拟内存区域在内核中是如何被组织的
...
或者 `pmap pid`
查看进程的虚拟内存空间布局以及其中包含的所有内存区域。这两个命令背后的实现原理就是通过遍历内核中的这个 vm_area_struct 双向链表获取的。
...
```
[#](https://www.xiaolincoding.com#_6-程序编译后的二进制文件如何映射到虚拟内存空间中) 6. 程序编译后的二进制文件如何映射到虚拟内存空间中
...
[#](https://www.xiaolincoding.com#_7-内核虚拟内存空间) 7. 内核虚拟内存空间
...
[#](https://www.xiaolincoding.com#_7-1-32-位体系内核虚拟内存空间布局) 7.1 32 位体系内核虚拟内存空间布局
在前边《5.1 内核如何划分用户态和内核态虚拟内存空间》小节中我们提到，内核在 `/arch/x86/include/asm/page_32_types.h`
文件中通过 TASK_SIZE 将进程虚拟内存空间和内核虚拟内存空间分割开来。
...
[#](https://www.xiaolincoding.com#_7-1-1-直接映射区) 7.1.1 直接映射区
...
[#](https://www.xiaolincoding.com#_7-1-2-zone-highmem-高端内存) 7.1.2 ZONE_HIGHMEM 高端内存
...
```
[#](https://www.xiaolincoding.com#_7-1-3-vmalloc-动态映射区) 7.1.3 vmalloc 动态映射区
...
[#](https://www.xiaolincoding.com#_7-1-4-永久映射区) 7.1.4 永久映射区
...
```
[#](https://www.xiaolincoding.com#_8-1-5-固定映射区) 8.1.5 固定映射区
...
[#](https://www.xiaolincoding.com#_7-1-6-临时映射区) 7.1.6 临时映射区
...
```
[#](https://www.xiaolincoding.com#_7-1-7-32位体系结构下-linux-虚拟内存空间整体布局) 7.1.7 32位体系结构下 Linux 虚拟内存空间整体布局
...
[#](https://www.xiaolincoding.com#_7-2-64-位体系内核虚拟内存空间布局) 7.2 64 位体系内核虚拟内存空间布局
...
在前边《5.1 内核如何划分用户态和内核态虚拟内存空间》小节中我们提到，内核在 `/arch/x86/include/asm/page_64_types.h`
文件中通过 TASK_SIZE 将进程虚拟内存空间和内核虚拟内存空间分割开来。
...
VMALLOC_START 和 VMALLOC_END 变量定义在 `/arch/x86/include/asm/pgtable_64_types.h`
文件中：
...
VMEMMAP_START 变量定义在 `/arch/x86/include/asm/pgtable_64_types.h`
文件中：
...
__START_KERNEL_map 变量定义在 `/arch/x86/include/asm/page_64_types.h`
文件中：

```
#define __START_KERNEL_map _AC(0xffffffff80000000, UL)
```
[#](https://www.xiaolincoding.com#_7-2-1-64位体系结构下-linux-虚拟内存空间整体布局) 7.2.1 64位体系结构下 Linux 虚拟内存空间整体布局
...
[#](https://www.xiaolincoding.com#_8-到底什么是物理内存地址) 8. 到底什么是物理内存地址
聊完了虚拟内存，我们接着聊一下物理内存，我们平时所称的内存也叫随机访问存储器（ random-access memory ）也叫 RAM 。而 RAM 分为两类：

一类是静态 RAM（ `SRAM`
），这类 SRAM 用于 CPU 高速缓存 L1Cache，L2Cache，L3Cache。其特点是访问速度快，访问速度为 1 - 30 个时钟周期，但是容量小，造价高。
-
另一类则是动态 RAM ( `DRAM`
...
[#](https://www.xiaolincoding.com#_8-1-dram-芯片的访问) 8.1 DRAM 芯片的访问
...
[#](https://www.xiaolincoding.com#_8-2-cpu-如何读写主存) 8.2 CPU 如何读写主存
...
[#](https://www.xiaolincoding.com#_8-3-cpu-从内存读取数据过程) 8.3 CPU 从内存读取数据过程
...
[#](https://www.xiaolincoding.com#_8-4-如何根据物理内存地址从主存中读取数据) 8.4 如何根据物理内存地址从主存中读取数据
...
[#](https://www.xiaolincoding.com#_8-5-cpu-向内存写入数据过程) 8.5 CPU 向内存写入数据过程
...
`cat /proc/pid/maps`
或者`pmap pid`
命令来查看进程用户态虚拟内存空间的实际分布。
还可以通过

`cat /proc/iomem`
命令来查看进程内核态虚拟内存空间的的实际分布。
