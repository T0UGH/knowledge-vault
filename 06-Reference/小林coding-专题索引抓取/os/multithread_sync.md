---
title: multithread_sync
source_url: https://www.xiaolincoding.com/os/4_process/multithread_sync.html
fetched_at: 2026-04-06
section: os
---

[#](https://www.xiaolincoding.com#_5-3-多线程冲突了怎么办) 5.3 多线程冲突了怎么办？
先来看看虚构的小故事
...
接下来，用 `30+`
张图，带大家走进操作系统中避免多线程资源竞争的**互斥、同步**的方法。

[#](https://www.xiaolincoding.com#竞争与协作) 竞争与协作
...
[#](https://www.xiaolincoding.com#互斥的概念) 互斥的概念
上面展示的情况称为**竞争条件（ race condition）**，当多线程相互竞争操作共享变量时，由于运气不好，即在执行过程中发生了上下文切换，我们得到了错误的结果，事实上，每次运行都可能得到不同的结果，因此输出的结果存在

**不确定性（**。
*indeterminate*）由于多线程执行操作共享变量的这段代码可能会导致竞争状态，因此我们将此段代码称为**临界区（ critical section），它是访问共享资源的代码片段，一定不能给多线程同时执行。**
...
[#](https://www.xiaolincoding.com#同步的概念) 同步的概念
...
-
[#](https://www.xiaolincoding.com#互斥与同步的实现和使用) 互斥与同步的实现和使用
...
[#](https://www.xiaolincoding.com#锁) 锁
...
`lock()`
，没有其他线程持有锁，所以`flag`
是 0。当调用`TestAndSet(flag, 1)`
方法，返回 0，线程会跳出 while 循环，获取锁。同时也会原子的设置 flag 为1，标志锁已经被持有。当线程离开临界区，调用`unlock()`
将`flag`
清理为 0。
...
会重复返回 1，本线程会一直**忙等**。当`flag`
终于被改为 0，本线程会调用`TestAndSet()`
...
[#](https://www.xiaolincoding.com#信号量) 信号量
信号量是操作系统提供的一种协调共享资源访问的方法。

通常**信号量表示资源的数量**，对应的变量是一个整型（`sem`
）变量。
...
[#](https://www.xiaolincoding.com#生产者-消费者问题) 生产者-消费者问题
生产者-消费者问题描述：
...
[#](https://www.xiaolincoding.com#经典同步问题) 经典同步问题
[#](https://www.xiaolincoding.com#哲学家就餐问题) 哲学家就餐问题
...
[#](https://www.xiaolincoding.com#读者-写者问题) 读者-写者问题
