---
title: grep表示只保留包含active的指标（忽略大小写）
source_url: https://www.xiaolincoding.com/os/3_memory/mem_reclaim.html
fetched_at: 2026-04-06
section: os
---

[#](https://www.xiaolincoding.com#_4-3-内存满了-会发生什么) 4.3 内存满了，会发生什么？
大家好，我是小林。
...
[#](https://www.xiaolincoding.com#内存分配的过程是怎样的) 内存分配的过程是怎样的？
...
[#](https://www.xiaolincoding.com#哪些内存可以被回收) 哪些内存可以被回收？
...
```
[#](https://www.xiaolincoding.com#回收内存带来的性能影响) 回收内存带来的性能影响
在前面我们知道了回收内存有两种方式。
...
[#](https://www.xiaolincoding.com#调整文件页和匿名页的回收倾向) 调整文件页和匿名页的回收倾向
...
[#](https://www.xiaolincoding.com#尽早触发-kswapd-内核线程异步回收内存) 尽早触发 kswapd 内核线程异步回收内存
...
**这时 kswapd0 会执行内存回收，直到剩余内存大于高阈值（pages_high）为止**。虽然会触发内存回收，但是不会阻塞应用程序，因为两者关系是异步的。
图中红色部分：如果剩余内存（pages_free）小于页最小阈值（pages_min），说明用户可用内存都耗尽了，此时就会

**触发直接内存回收**，这时应用程序就会被阻塞，因为两者关系是同步的。
...
[#](https://www.xiaolincoding.com#numa-架构下的内存回收策略) NUMA 架构下的内存回收策略
什么是 NUMA 架构？
...
[#](https://www.xiaolincoding.com#如何保护一个进程不被-oom-杀掉呢) 如何保护一个进程不被 OOM 杀掉呢？
...
第一，进程已经使用的物理内存页面数。
-
第二，每个进程的 OOM 校准值 oom_score_adj。它是可以通过 `/proc/[pid]/oom_score_adj`
