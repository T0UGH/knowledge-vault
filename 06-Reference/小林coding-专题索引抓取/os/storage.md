---
title: storage
source_url: https://www.xiaolincoding.com/os/1_hardware/storage.html
fetched_at: 2026-04-06
section: os
---

[#](https://www.xiaolincoding.com#_2-2-磁盘比内存慢几万倍) 2.2 磁盘比内存慢几万倍？
...
[#](https://www.xiaolincoding.com#存储器的层次结构) 存储器的层次结构
...
SSD/HDD 硬盘
-
[#](https://www.xiaolincoding.com#寄存器) 寄存器
...
[#](https://www.xiaolincoding.com#cpu-cache) CPU Cache
...
[#](https://www.xiaolincoding.com#l1-高速缓存) L1 高速缓存
...
32K
```
[#](https://www.xiaolincoding.com#l2-高速缓存) L2 高速缓存
...
256K
```
[#](https://www.xiaolincoding.com#l3-高速缓存) L3 高速缓存
...
DRAM 的数据访问电路和刷新电路都比 SRAM 更复杂，所以访问的速度会更慢，内存速度大概在 `200~300`
个 时钟周期之间。

[#](https://www.xiaolincoding.com#ssd-hdd-硬盘) SSD/HDD 硬盘
SSD（*Solid-state disk*） 就是我们常说的固体硬盘，结构和内存类似，但是它相比内存的优点是断电后数据还是存在的，而内存、寄存器、高速缓存断电后数据都会丢失。内存的读写速度比 SSD 大概快 `10~1000`
倍。
...
[#](https://www.xiaolincoding.com#存储器的层次关系) 存储器的层次关系
...
[#](https://www.xiaolincoding.com#存储器之间的实际价格和性能差距) 存储器之间的实际价格和性能差距
前面我们知道了，速度越快的存储器，造价成本往往也越高，那我们就以实际的数据来看看，不同层级的存储器之间的性能和价格差异。

下面这张表格是不同层级的存储器之间的成本对比图：

你可以看到 L1 Cache 的访问延时是 1 纳秒，而内存已经是 100 纳秒了，相比 L1 Cache 速度慢了 `100`
倍。另外，机械硬盘的访问延时更是高达 10 毫秒，相比 L1 Cache 速度慢了 `10000000`
倍，差了好几个数量级别。

在价格上，每生成 MB 大小的 L1 Cache 相比内存贵了 `466`
...
再来回答，开头的问题：那机械硬盘、固态硬盘、内存这三个存储器，到底和 `CPU L1 Cache`
相比速度差多少倍呢？
