---
title: device
source_url: https://www.xiaolincoding.com/os/7_device/device.html
fetched_at: 2026-04-06
section: os
---

[#](https://www.xiaolincoding.com#_8-1-键盘敲入-a-字母时-操作系统期间发生了什么) 8.1 键盘敲入 A 字母时，操作系统期间发生了什么？
...
[#](https://www.xiaolincoding.com#设备控制器) 设备控制器
...
-
实际上，控制器是有三类寄存器，它们分别是**状态寄存器（ Status Register）**、

**命令寄存器（**，如下图：
*Command Register*）**以及**数据寄存器（*Data Register*）这三个寄存器的作用：
...
*端口 I/O*，每个控制寄存器被分配一个 I/O 端口，可以通过特殊的汇编指令操作这些寄存器，比如`in/out`
类似的指令。
*内存映射 I/O*，将所有控制寄存器映射到内存空间中，这样就可以像读写内存一样读写数据缓冲区。
[#](https://www.xiaolincoding.com#i-o-控制方式) I/O 控制方式
...
另外，中断有两种，一种**软中断**，例如代码调用 `INT`
指令触发，一种是**硬件中断**，就是硬件通过中断控制器触发的。
...
[#](https://www.xiaolincoding.com#设备驱动程序) 设备驱动程序
...
恢复被中断进程的上下文；
-
[#](https://www.xiaolincoding.com#通用块层) 通用块层
...
[#](https://www.xiaolincoding.com#存储系统-i-o-软件分层) 存储系统 I/O 软件分层
...
-
[#](https://www.xiaolincoding.com#键盘敲入字母时-期间发生了什么) 键盘敲入字母时，期间发生了什么？
