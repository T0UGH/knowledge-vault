---
title: https_rsa
source_url: https://www.xiaolincoding.com/network/2_http/https_rsa.html
fetched_at: 2026-04-06
section: network
---

[#](https://www.xiaolincoding.com#_3-3-https-rsa-握手解析) 3.3 HTTPS RSA 握手解析
...
[#](https://www.xiaolincoding.com#tls-握手过程) TLS 握手过程
...
[#](https://www.xiaolincoding.com#rsa-握手过程) RSA 握手过程
...
[#](https://www.xiaolincoding.com#tls-第一次握手) TLS 第一次握手
客户端首先会发一个「**Client Hello**」消息，字面意思我们也能理解到，这是跟服务器「打招呼」。

消息里面有客户端使用的 TLS 版本号、支持的密码套件列表，以及生成的**随机数（ Client Random）**，这个随机数会被服务端保留，它是生成对称加密密钥的材料之一。

[#](https://www.xiaolincoding.com#tls-第二次握手) TLS 第二次握手
...
[#](https://www.xiaolincoding.com#客户端验证证书) 客户端验证证书
在这里刹个车，客户端拿到了服务端的数字证书后，要怎么校验该数字证书是真实有效的呢？

[#](https://www.xiaolincoding.com#数字证书和-ca-机构) 数字证书和 CA 机构
...
[#](https://www.xiaolincoding.com#数字证书签发和验证流程) 数字证书签发和验证流程
如下图图所示，为数字证书签发和验证流程：
...
-
[#](https://www.xiaolincoding.com#证书链) 证书链
...
[#](https://www.xiaolincoding.com#tls-第三次握手) TLS 第三次握手
客户端验证完证书后，认为可信则继续往下走。

接着，客户端就会生成一个新的**随机数 ( pre-master)**，用服务器的 RSA 公钥加密该随机数，通过「

**Client Key Exchange**」消息传给服务端。
...
[#](https://www.xiaolincoding.com#tls-第四次握手) TLS 第四次握手
服务器也是同样的操作，发「**Change Cipher Spec**」和「**Encrypted Handshake Message**」消息，如果双方都验证加密和解密没问题，那么握手正式完成。

最后，就用「会话密钥」加解密 HTTP 请求和响应了。

[#](https://www.xiaolincoding.com#rsa-算法的缺陷) RSA 算法的缺陷
