---
title: ip_base
source_url: https://www.xiaolincoding.com/network/4_ip/ip_base.html
fetched_at: 2026-04-06
section: network
---

[#](https://www.xiaolincoding.com#_5-1-ip-基础知识全家桶) 5.1 IP 基础知识全家桶
...
菜狗（押韵不？）
[#](https://www.xiaolincoding.com#前菜-ip-基本认识) 前菜 —— IP 基本认识
...
[#](https://www.xiaolincoding.com#主菜-ip-地址的基础知识) 主菜 —— IP 地址的基础知识
在 TCP/IP 网络通信时，为了保证能正常通信，每个设备都需要配置正确的 IP 地址，否则无法实现正常的通信。

IP 地址（IPv4 地址）由 `32`
位正整数来表示，IP 地址在计算机是以二进制的方式处理的。

而人类为了方便记忆采用了**点分十进制**的标记方式，也就是将 32 位 IP 地址以每 8 位为组，共分为 `4`
组，每组以「`.`
」隔开，再将每组转换成十进制。
...
[#](https://www.xiaolincoding.com#ip-地址的分类) IP 地址的分类
...
[#](https://www.xiaolincoding.com#无分类地址-cidr) 无分类地址 CIDR
...
[#](https://www.xiaolincoding.com#公有-ip-地址与私有-ip-地址) 公有 IP 地址与私有 IP 地址
...
[#](https://www.xiaolincoding.com#ip-地址与路由控制) IP 地址与路由控制
...
主机 A 要发送一个 IP 包，其源地址是 `10.1.1.30`
和目标地址是`10.1.2.10`
，由于没有在主机 A 的路由表找到与目标地址`10.1.2.10`
相同的网络地址，于是包被转发到默认路由（路由器`1`
）
-
路由器 `1`
收到 IP 包后，也在路由器`1`
的路由表匹配与目标地址相同的网络地址记录，发现匹配到了，于是就把 IP 数据包转发到了`10.1.0.2`
这台路由器`2`
-
路由器 `2`
收到后，同样对比自身的路由表，发现匹配到了，于是把 IP 包从路由器`2`
的`10.1.2.1`
...
[#](https://www.xiaolincoding.com#ip-分片与重组) IP 分片与重组
每种数据链路的最大传输单元 `MTU`
...
[#](https://www.xiaolincoding.com#ipv6-基本认识) IPv6 基本认识
...
-
IPv6 包头包首部长度采用固定的值 `40`
字节，去掉了包头校验和，简化了首部结构，减轻了路由器负荷，大大**提高了传输的性能**。
-
...
-
[#](https://www.xiaolincoding.com#ipv4-首部与-ipv6-首部) IPv4 首部与 IPv6 首部
...
**取消选项字段。**选项字段不再是标准 IP 首部的一部分了，但它并没有消失，而是可能出现在 IPv6 首部中的「下一个首部」指出的位置上。删除该选项字段使的 IPv6 的首部成为固定长度的`40`
字节。
[#](https://www.xiaolincoding.com#点心-ip-协议相关技术) 点心 —— IP 协议相关技术
...
IGMP 因特网组管理协议
-
[#](https://www.xiaolincoding.com#dns) DNS
...
DNS 中的域名都是用**句点**来分隔的，比如 `www.server.com`
，这里的句点代表了不同层次之间的**界限**。
...
[#](https://www.xiaolincoding.com#arp) ARP
...
[#](https://www.xiaolincoding.com#dhcp) DHCP
...
DHCP 客户端会向 DHCP 中继代理发送 DHCP 请求包，而 DHCP 中继代理在收到这个广播包以后，再以
...
[#](https://www.xiaolincoding.com#nat) NAT
...
[#](https://www.xiaolincoding.com#icmp) ICMP
...
-
[#](https://www.xiaolincoding.com#igmp) IGMP
...
接下来，以 `IGMPv2`
作为例子，说说**常规查询与响应和离开组播组**这两个工作机制。

*常规查询与响应工作机制*
路由器会周期性发送目的地址为 `224.0.0.1`
