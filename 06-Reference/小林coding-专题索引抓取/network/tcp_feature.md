---
title: 1 是已发送并收到 ACK确认的数据：1~31 字节
source_url: https://www.xiaolincoding.com/network/3_tcp/tcp_feature.html
fetched_at: 2026-04-06
section: network
---

[#](https://www.xiaolincoding.com#_4-2-tcp-重传、滑动窗口、流量控制、拥塞控制) 4.2 TCP 重传、滑动窗口、流量控制、拥塞控制
...
[#](https://www.xiaolincoding.com#重传机制) 重传机制
...
D-SACK
-
[#](https://www.xiaolincoding.com#超时重传) 超时重传
重传机制的其中一个方式，就是在发送数据时，设定一个定时器，当超过指定的时间后，没有收到对方的 `ACK`
确认应答报文，就会重发该数据，也就是我们常说的**超时重传**。
...
[#](https://www.xiaolincoding.com#快速重传) 快速重传
...
[#](https://www.xiaolincoding.com#sack-方法) SACK 方法
还有一种实现重传机制的方式叫：`SACK`
...
[#](https://www.xiaolincoding.com#duplicate-sack) Duplicate SACK
...
[#](https://www.xiaolincoding.com#滑动窗口) 滑动窗口
引入窗口概念的原因
...
[#](https://www.xiaolincoding.com#流量控制) 流量控制
...
-
服务端收到请求报文后，发送确认报文和 80 字节的数据，于是可用窗口 `Usable`
减少为 120 字节，同时`SND.NXT`
指针也向右偏移 80 字节后，指向 321，**这意味着下次发送数据的时候，序列号是 321。**
-
客户端收到 80 字节数据后，于是接收窗口往右移动 80 字节，`RCV.NXT`
也就指向 321，**这意味着客户端期望的下一个报文的序列号是 321**，接着发送确认报文给服务端。
-
服务端再次发送了 120 字节数据，于是可用窗口耗尽为 0，服务端无法再继续发送数据。
-
客户端收到 120 字节的数据后，于是接收窗口往右移动 120 字节，`RCV.NXT`
也就指向 441，接着发送确认报文给服务端。
-
服务端收到对 80 字节数据的确认报文后，`SND.UNA`
指针往右偏移后指向 321，于是可用窗口`Usable`
增大到 80。
-
服务端收到对 120 字节数据的确认报文后，`SND.UNA`
指针往右偏移后指向 441，于是可用窗口`Usable`
增大到 200。
-
服务端可以继续发送了，于是发送了 160 字节的数据后，`SND.NXT`
指向 601，于是可用窗口`Usable`
减少到 40。
-
客户端收到 160 字节后，接收窗口往右移动了 160 字节，`RCV.NXT`
也就是指向了 601，接着发送确认报文给服务端。
-
服务端收到对 160 字节数据的确认报文后，发送窗口往右移动了 160 字节，于是 `SND.UNA`
指针偏移了 160 后指向 601，可用窗口`Usable`
也就增大至了 200。
-
[#](https://www.xiaolincoding.com#操作系统缓冲区与滑动窗口的关系) 操作系统缓冲区与滑动窗口的关系
...
[#](https://www.xiaolincoding.com#窗口关闭) 窗口关闭
...
[#](https://www.xiaolincoding.com#糊涂窗口综合症) 糊涂窗口综合症
...
```
[#](https://www.xiaolincoding.com#拥塞控制) 拥塞控制
...
快速恢复
-
[#](https://www.xiaolincoding.com#慢启动) 慢启动
...
时，就会使用「拥塞避免算法」。
-
[#](https://www.xiaolincoding.com#拥塞避免算法) 拥塞避免算法
前面说道，当拥塞窗口 `cwnd`
「超过」慢启动门限 `ssthresh`
就会进入拥塞避免算法。
...
[#](https://www.xiaolincoding.com#拥塞发生) 拥塞发生
...
进入快速恢复算法
-
[#](https://www.xiaolincoding.com#快速恢复) 快速恢复
...
是 1/cwnd，你可以在 RFC2581 第 3 页找到答案
-
没有写反，同样你可以在 RFC2581 第 5 页找到答案
-
ssthresh 就是慢启动门限，我觉得 ssthresh 我已经说的很清楚了，当然你可以找其他资料补充你的疑惑
-
是吧？ TCP 巨复杂吧？看完很累吧？
