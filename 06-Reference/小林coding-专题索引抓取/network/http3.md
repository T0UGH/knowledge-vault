---
title: http3
source_url: https://www.xiaolincoding.com/network/2_http/http3.html
fetched_at: 2026-04-06
section: network
---

[#](https://www.xiaolincoding.com#_3-7-http-3-强势来袭) 3.7 HTTP/3 强势来袭
HTTP/3 现在（2022 年 5 月）还没正式推出，不过自 2017 年起，HTTP/3 已经更新到 34 个草案了，基本的特性已经确定下来了，对于包格式可能后续会有变化。

所以，这次 HTTP/3 介绍不会涉及到包格式，只说它的特性。

[#](https://www.xiaolincoding.com#美中不足的-http-2) 美中不足的 HTTP/2
...
网络迁移需要重新连接；
-
[#](https://www.xiaolincoding.com#队头阻塞) 队头阻塞
...
[#](https://www.xiaolincoding.com#tcp-与-tls-的握手时延迟) TCP 与 TLS 的握手时延迟
发起 HTTP 请求时，需要经过 TCP 三次握手和 TLS 四次握手（TLS 1.2）的过程，因此共需要 3 个 RTT 的时延才能发出请求数据。

另外，TCP 由于具有「拥塞控制」的特性，所以刚建立连接的 TCP 会有个「慢启动」的过程，它会对 TCP 连接产生“减速”效果。

[#](https://www.xiaolincoding.com#网络迁移需要重新连接) 网络迁移需要重新连接
一个 TCP 连接是由四元组（源 IP 地址，源端口，目标 IP 地址，目标端口）确定的，这意味着如果 IP 地址或者端口变动了，就会导致需要 TCP 与 TLS 重新握手，这不利于移动设备切换网络的场景，比如 4G 网络环境切换成 WiFi。

这些问题都是 TCP 协议固有的问题，无论应用层的 HTTP/2 在怎么设计都无法逃脱。要解决这个问题，就必须把**传输层协议替换成 UDP**，这个大胆的决定，HTTP/3 做了！

[#](https://www.xiaolincoding.com#quic-协议的特点) QUIC 协议的特点
...
连接迁移；
-
[#](https://www.xiaolincoding.com#无队头阻塞) 无队头阻塞
...
[#](https://www.xiaolincoding.com#更快的连接建立) 更快的连接建立
...
[#](https://www.xiaolincoding.com#连接迁移) 连接迁移
...
[#](https://www.xiaolincoding.com#http-3-协议) HTTP/3 协议
