---
title: http2
source_url: https://www.xiaolincoding.com/network/2_http/http2.html
fetched_at: 2026-04-06
section: network
---

[#](https://www.xiaolincoding.com#_3-6-http-2-牛逼在哪) 3.6 HTTP/2 牛逼在哪？
不多 BB 了，直接发车！

**一起来看看 HTTP/2 牛逼在哪？**
[#](https://www.xiaolincoding.com#http-1-1-协议的性能问题) HTTP/1.1 协议的性能问题
...
[#](https://www.xiaolincoding.com#兼容-http-1-1) 兼容 HTTP/1.1
...
[#](https://www.xiaolincoding.com#头部压缩) 头部压缩
...
[#](https://www.xiaolincoding.com#静态表编码) 静态表编码
HTTP/2 为高频出现在头部的字符串和字段建立了一张**静态表**，它是写入到 HTTP/2 框架里的，不会变化的，静态表里共有 `61`
组，如下图：
...
于是，在统计大量的 HTTP 头部后，HTTP/2 根据出现频率将 ASCII 码编码为了 Huffman 编码表，可以在 RFC7541 文档找到这张**静态 Huffman 表**，我就不把表的全部内容列出来了，我只列出字符串 `nghttpx`
...
[#](https://www.xiaolincoding.com#动态表编码) 动态表编码
静态表只包含了 61 种高频出现在头部的字符串，不在静态表范围内的头部字符串就要自行构建**动态表**，它的 Index 从 `62`
起步，会在编码解码的时候随时更新。
...
[#](https://www.xiaolincoding.com#二进制帧) 二进制帧
...
[#](https://www.xiaolincoding.com#并发传输) 并发传输
...
在 Nginx 中，可以通过 `http2_max_concurrent_Streams`
配置来设置 Stream 的上限，默认是 128 个。
...
[#](https://www.xiaolincoding.com#服务器主动推送资源) 服务器主动推送资源
