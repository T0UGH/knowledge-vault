...
source_url: https://www.xiaolincoding.com/network/1_base/what_happen_url.html
fetched_at: 2026-04-06
section: network
---

[#](https://www.xiaolincoding.com#_2-2-键入网址到网页显示-期间发生了什么) 2.2 键入网址到网页显示，期间发生了什么？
...
[#](https://www.xiaolincoding.com#孤单小弟-http) 孤单小弟 —— HTTP
浏览器做的第一步工作是解析 URL
...
当没有路径名时，就代表访问根目录下事先设置的**默认文件**，也就是 `/index.html`
或者 `/default.html`
这些文件，这样就不会发生混乱了。
...
[#](https://www.xiaolincoding.com#真实地址查询-dns) 真实地址查询 —— DNS
...
[#](https://www.xiaolincoding.com#指南好帮手-协议栈) 指南好帮手 —— 协议栈
...
[#](https://www.xiaolincoding.com#可靠传输-tcp) 可靠传输 —— TCP
...
[#](https://www.xiaolincoding.com#远程定位-ip) 远程定位 —— IP
...
-
因为 HTTP 是经过 TCP 传输的，所以在 IP 包头的**协议号**，要填写为 `06`
（十六进制），表示协议为 TCP。
...
首先先和第一条目的子网掩码（`Genmask`
）进行**与运算**，得到结果为`192.168.10.0`
，但是第一个条目的`Destination`
是`192.168.3.0`
，两者不一致所以匹配失败。
-
再与第二条目的子网掩码进行 **与运算**，得到的结果为`192.168.10.0`
，与第二条目的`Destination 192.168.10.0`
匹配成功，所以将使用`eth1`
...
，这表示**默认网关**，如果其他所有条目都无法匹配，就会自动匹配这一行。并且后续就把包发给路由器，`Gateway`
即是路由器的 IP 地址。
...
[#](https://www.xiaolincoding.com#两点传输-mac) 两点传输 —— MAC
...
所以先得搞清楚应该把包发给谁，这个只要查一下**路由表**就知道了。在路由表中找到相匹配的条目，然后把包发给 `Gateway`
列中的 IP 地址就可以了。
...
[#](https://www.xiaolincoding.com#出口-网卡) 出口 —— 网卡
...
[#](https://www.xiaolincoding.com#送别者-交换机) 送别者 —— 交换机
...
[#](https://www.xiaolincoding.com#出境大门-路由器) 出境大门 —— 路由器
路由器与交换机的区别
...
IP 做 **& 与运算**后，得到的结果与对应条目的目标地址进行匹配，如果匹配就会作为候选转发目标，如果不匹配就继续与下个条目进行路由匹配。

如第二条目的子网掩码 `255.255.255.0`
与 `192.168.1.100`
IP 做 **& 与运算**后，得到结果是 `192.168.1.0`
，这与第二条目的目标地址 `192.168.1.0`
匹配，该第二条目记录就会被作为转发目标。

实在找不到匹配路由时，就会选择**默认路由**，路由表中子网掩码为 `0.0.0.0`
的记录表示「默认路由」。
...
[#](https://www.xiaolincoding.com#互相扒皮-服务器-与-客户端) 互相扒皮 —— 服务器 与 客户端
...
[#](https://www.xiaolincoding.com#一个数据包臭不要脸的感受) 一个数据包臭不要脸的感受
