---
title: sentinel
source_url: https://www.xiaolincoding.com/redis/cluster/sentinel.html
fetched_at: 2026-04-06
section: redis
---

[#](https://www.xiaolincoding.com#为什么要有哨兵) 为什么要有哨兵？
大家好，我是小林。

这次聊聊，Redis 的哨兵机制。

[#](https://www.xiaolincoding.com#为什么要有哨兵机制) 为什么要有哨兵机制？
...
Redis 在 2.8 版本以后提供的**哨兵（ Sentinel）机制**，它的作用是实现

**主从节点故障转移**。它会监测主节点是否存活，如果发现主节点挂了，它就会选举一个从节点切换为主节点，并且把新主节点的相关信息通知给从节点和客户端。
[#](https://www.xiaolincoding.com#哨兵机制是如何工作的) 哨兵机制是如何工作的？
...
-
[#](https://www.xiaolincoding.com#如何判断主节点真的故障了) 如何判断主节点真的故障了？
...
[#](https://www.xiaolincoding.com#由哪个哨兵进行主从故障转移) 由哪个哨兵进行主从故障转移？
...
[#](https://www.xiaolincoding.com#主从故障转移的过程是怎样的) 主从故障转移的过程是怎样的？
...
-
[#](https://www.xiaolincoding.com#步骤一-选出新主节点) 步骤一：选出新主节点
...
-
[#](https://www.xiaolincoding.com#第一轮考察-优先级最高的从节点胜出) 第一轮考察：优先级最高的从节点胜出
...
[#](https://www.xiaolincoding.com#第二轮考察-复制进度最靠前的从节点胜出) 第二轮考察：复制进度最靠前的从节点胜出
...
[#](https://www.xiaolincoding.com#第三轮考察-id-号小的从节点胜出) 第三轮考察：ID 号小的从节点胜出
...
[#](https://www.xiaolincoding.com#步骤二-将从节点指向新主节点) 步骤二：将从节点指向新主节点
当新主节点出现之后，哨兵 leader 下一步要做的就是，让已下线主节点属下的所有「从节点」指向「新主节点」，这一动作可以通过向「从节点」发送 `SLAVEOF`
命令来实现。

如下图，哨兵 leader 向所有从节点（server3 和 server4）发送 `SLAVEOF`
，让它们成为新主节点的从节点。

所有从节点指向新主节点后的拓扑图如下：

[#](https://www.xiaolincoding.com#步骤三-通知客户的主节点已更换) 步骤三：通知客户的主节点已更换
...
[#](https://www.xiaolincoding.com#步骤四-将旧主节点变为从节点) 步骤四：将旧主节点变为从节点
故障转移操作最后要做的是，继续监视旧主节点，当旧主节点重新上线时，哨兵集群就会向它发送 `SLAVEOF`
命令，让它成为新主节点的从节点，如下图：

至此，整个主从节点的故障转移的工作结束。

[#](https://www.xiaolincoding.com#哨兵集群是如何组成的) 哨兵集群是如何组成的？
...
在主从集群中，主节点上有一个名为`__sentinel__:hello`
的频道，不同哨兵就是通过它来相互发现，实现互相通信的。

在下图中，哨兵 A 把自己的 IP 地址和端口的信息发布到`__sentinel__:hello`
