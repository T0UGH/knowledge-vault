...
source_url: https://www.xiaolincoding.com/redis/data_struct/data_struct.html
...
[#](https://www.xiaolincoding.com#键值对数据库是怎么实现的) 键值对数据库是怎么实现的？
...
[#](https://www.xiaolincoding.com#sds) SDS
...
[#](https://www.xiaolincoding.com#c-语言字符串的缺陷) C 语言字符串的缺陷
...
[#](https://www.xiaolincoding.com#sds-结构设计) SDS 结构设计
下图就是 Redis 5.0 的 SDS 的数据结构：

结构中的每个成员变量分别介绍下：

**len，记录了字符串长度**。这样获取字符串长度的时候，只需要返回这个成员变量值就行，时间复杂度只需要 O（1）。
**alloc，分配给字符数组的空间长度**。这样在修改字符串的时候，可以通过`alloc - len`
...
[#](https://www.xiaolincoding.com#o-1-复杂度获取字符串长度) O（1）复杂度获取字符串长度
C 语言的字符串长度获取 strlen 函数，需要通过遍历的方式来统计字符串长度，时间复杂度是 O（N）。

而 Redis 的 SDS 结构因为加入了 len 成员变量，那么**获取字符串长度的时候，直接返回这个成员变量的值就行，所以复杂度只有 O（1）**。

[#](https://www.xiaolincoding.com#二进制安全) 二进制安全
...
[#](https://www.xiaolincoding.com#不会发生缓冲区溢出) 不会发生缓冲区溢出
...
[#](https://www.xiaolincoding.com#节省内存空间) 节省内存空间
...
如果不想编译器使用字节对齐的方式进行分配内存，可以采用了 `__attribute__ ((packed))`
...
[#](https://www.xiaolincoding.com#链表结构设计) 链表结构设计
...
[#](https://www.xiaolincoding.com#链表的优势与缺陷) 链表的优势与缺陷
Redis 的链表实现优点如下：
...
**无法很好利用 CPU 缓存**。能很好利用 CPU 缓存的数据结构就是数组，因为数组的内存是连续的，这样就可以充分利用 CPU 缓存来加速访问。
...
[#](https://www.xiaolincoding.com#压缩列表) 压缩列表
...
[#](https://www.xiaolincoding.com#压缩列表结构设计) 压缩列表结构设计
压缩列表是 Redis 为了节约内存而开发的，它是**由连续内存块组成的顺序型数据结构**，有点类似于数组。

压缩列表在表头有三个字段：

，记录整个压缩列表占用对内存字节数；**zlbytes**
，记录压缩列表「尾部」节点距离起始地址有多少字节，也就是列表尾的偏移量；**zltail**
，记录压缩列表包含的节点数量；**zllen**
，标记压缩列表的结束点，固定值 0xFF（十进制255）。**zlend**
...
，记录了「前一个节点」的长度，目的是为了实现从后向前遍历；**prevlen**
，记录了当前节点实际数据的「类型和长度」，类型主要有两种：字符串和整数。**encoding**
，记录了当前节点的实际数据，类型和长度都由**data**`encoding`
决定；
...
-
[#](https://www.xiaolincoding.com#连锁更新) 连锁更新
...
[#](https://www.xiaolincoding.com#压缩列表的缺陷) 压缩列表的缺陷
...
[#](https://www.xiaolincoding.com#哈希表) 哈希表
...
[#](https://www.xiaolincoding.com#哈希表结构设计) 哈希表结构设计
Redis 的哈希表结构如下：
...
[#](https://www.xiaolincoding.com#哈希冲突) 哈希冲突
...
[#](https://www.xiaolincoding.com#链式哈希) 链式哈希
...
[#](https://www.xiaolincoding.com#rehash) rehash
...
[#](https://www.xiaolincoding.com#渐进式-rehash) 渐进式 rehash
...
[#](https://www.xiaolincoding.com#rehash-触发条件) rehash 触发条件
...
[#](https://www.xiaolincoding.com#整数集合) 整数集合
整数集合是 Set 对象的底层实现之一。当一个 Set 对象只包含整数值元素，并且元素数量不大时，就会使用整数集这个数据结构作为底层实现。

[#](https://www.xiaolincoding.com#整数集合结构设计) 整数集合结构设计
...
[#](https://www.xiaolincoding.com#整数集合的升级操作) 整数集合的升级操作
...
[#](https://www.xiaolincoding.com#跳表) 跳表
...
[#](https://www.xiaolincoding.com#跳表结构设计) 跳表结构设计
...
-
[#](https://www.xiaolincoding.com#跳表节点查询过程) 跳表节点查询过程
...
-
[#](https://www.xiaolincoding.com#跳表节点层数设置) 跳表节点层数设置
...
[#](https://www.xiaolincoding.com#为什么用跳表而不用平衡树) 为什么用跳表而不用平衡树？
...
[#](https://www.xiaolincoding.com#quicklist) quicklist
...
[#](https://www.xiaolincoding.com#quicklist-结构设计) quicklist 结构设计
...
[#](https://www.xiaolincoding.com#listpack) listpack
...
[#](https://www.xiaolincoding.com#listpack-结构设计) listpack 结构设计
