---
title: 设置键值对的时候，同时指定过期时间位 60 秒
source_url: https://www.xiaolincoding.com/redis/module/strategy.html
fetched_at: 2026-04-06
section: redis
---

[#](https://www.xiaolincoding.com#redis-过期删除策略和内存淘汰策略有什么区别) Redis 过期删除策略和内存淘汰策略有什么区别？
大家好，我是小林。
...
[#](https://www.xiaolincoding.com#过期删除策略) 过期删除策略
Redis 是可以对 key 设置过期时间的，因此需要有相应的机制将已过期的键值对删除，而做这个工作的就是过期键值删除策略。

[#](https://www.xiaolincoding.com#如何设置过期时间) 如何设置过期时间？
...
(integer) -1
```
[#](https://www.xiaolincoding.com#如何判定-key-已过期了) 如何判定 key 已过期了？
...
[#](https://www.xiaolincoding.com#过期删除策略有哪些) 过期删除策略有哪些？
...
-
[#](https://www.xiaolincoding.com#redis-过期删除策略是什么) Redis 过期删除策略是什么？
...
[#](https://www.xiaolincoding.com#内存淘汰策略) 内存淘汰策略
前面说的过期删除策略，是删除已过期的 key，而当 Redis 的运行内存已经超过 Redis 设置的最大内存之后，则会使用内存淘汰策略删除符合条件的 key，以此来保障 Redis 高效的运行。

[#](https://www.xiaolincoding.com#如何设置-redis-最大运行内存) 如何设置 Redis 最大运行内存？
...
方式一：通过“`config set maxmemory-policy <策略>`
”命令设置。它的优点是设置之后立即生效，不需要重启 Redis 服务，缺点是重启 Redis 之后，设置就会失效。
-
