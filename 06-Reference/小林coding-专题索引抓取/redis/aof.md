---
title: aof
source_url: https://www.xiaolincoding.com/redis/storage/aof.html
fetched_at: 2026-04-06
section: redis
---

[#](https://www.xiaolincoding.com#aof-持久化是怎么实现的) AOF 持久化是怎么实现的？
[#](https://www.xiaolincoding.com#aof-日志) AOF 日志
...
命令查看里面的内容，不过里面的内容如果不知道一定的规则的话，可能会看不懂。
...
[#](https://www.xiaolincoding.com#三种写回策略) 三种写回策略
Redis 写入 AOF 日志的过程，如下图：

我先来具体说说：

Redis 执行完写操作命令后，会将命令追加到 `server.aof_buf`
缓冲区；
-
...
-
[#](https://www.xiaolincoding.com#aof-重写机制) AOF 重写机制
...
[#](https://www.xiaolincoding.com#aof-后台重写) AOF 后台重写
...
主进程在通过 `fork`
系统调用生成 bgrewriteaof 子进程时，操作系统会把主进程的「**页表**」复制一份给子进程，这个页表记录着虚拟地址和物理地址映射关系，而不会复制物理内存，也就是说，两者的虚拟空间不同，但其对应的物理空间是同一个。
