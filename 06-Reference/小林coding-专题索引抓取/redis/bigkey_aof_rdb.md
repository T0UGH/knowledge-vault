---
title: 最近一次 fork 操作耗时
source_url: https://www.xiaolincoding.com/redis/storage/bigkey_aof_rdb.html
fetched_at: 2026-04-06
section: redis
---

[#](https://www.xiaolincoding.com#redis-大-key-对持久化有什么影响) Redis 大 Key 对持久化有什么影响？
大家好，我是小林。
...
[#](https://www.xiaolincoding.com#大-key-对-aof-日志的影响) 大 Key 对 AOF 日志的影响
先说说 AOF 日志三种写回磁盘的策略
...
[#](https://www.xiaolincoding.com#大-key-对-aof-重写和-rdb-的影响) 大 Key 对 AOF 重写和 RDB 的影响
...
在通过 `fork()`
函数创建子进程的时候，虽然不会复制父进程的物理内存，但是**内核会把父进程的页表复制一份给子进程，如果页表很大，那么这个复制过程是会很耗时的，那么在执行 fork 函数的时候就会发生阻塞现象**。

而且，fork 函数是由 Redis 主线程调用的，如果 fork 函数发生阻塞，那么意味着就会阻塞 Redis 主线程。由于 Redis 执行命令是在主线程处理的，所以当 Redis 主线程发生阻塞，就无法处理后续客户端发来的命令。

我们可以执行 `info`
命令获取到 latest_fork_usec 指标，表示 Redis 最近一次 fork 操作耗时。
