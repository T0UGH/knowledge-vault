---
title: 删除缓存
source_url: https://www.xiaolincoding.com/redis/architecture/mysql_redis_consistency.html
fetched_at: 2026-04-06
section: redis
---

[#](https://www.xiaolincoding.com#数据库和缓存如何保证一致性) 数据库和缓存如何保证一致性？
...
[#](https://www.xiaolincoding.com#先更新数据库-还是先更新缓存) 先更新数据库，还是先更新缓存？
...
[#](https://www.xiaolincoding.com#先更新数据库-再更新缓存) 先更新数据库，再更新缓存
...
[#](https://www.xiaolincoding.com#先更新缓存-再更新数据库) 先更新缓存，再更新数据库
...
[#](https://www.xiaolincoding.com#先更新数据库-还是先删除缓存) 先更新数据库，还是先删除缓存？
...
[#](https://www.xiaolincoding.com#先删除缓存-再更新数据库) 先删除缓存，再更新数据库
阿旺还是以用户表的场景来分析。
...
[#](https://www.xiaolincoding.com#先更新数据库-再删除缓存) 先更新数据库，再删除缓存
继续用「读 + 写」请求的并发的场景来分析。
...
[#](https://www.xiaolincoding.com#前情回顾) 前情回顾
...
老板画的饼事情，能否兑现给阿旺呢？
-
[#](https://www.xiaolincoding.com#如何保证两个操作都能执行成功) 如何保证两个操作都能执行成功？
...
[#](https://www.xiaolincoding.com#消息队列重试机制) 消息队列重试机制
...
[#](https://www.xiaolincoding.com#订阅-mysql-binlog-再操作缓存) 订阅 MySQL binlog，再操作缓存
...
[#](https://www.xiaolincoding.com#老板发饼啦) 老板发饼啦
...
*以上故事纯属虚拟，如有巧合，以你为准*。
[#](https://www.xiaolincoding.com#读者提问) 读者提问
为什么是删除缓存，而不是更新缓存呢？
