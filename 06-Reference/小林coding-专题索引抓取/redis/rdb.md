---
title: rdb
source_url: https://www.xiaolincoding.com/redis/storage/rdb.html
fetched_at: 2026-04-06
section: redis
---

[#](https://www.xiaolincoding.com#rdb-快照是怎么实现的) RDB 快照是怎么实现的？
大家好，我是小林哥。
...
[#](https://www.xiaolincoding.com#快照怎么用) 快照怎么用？
...
[#](https://www.xiaolincoding.com#执行快照时-数据能被修改吗) 执行快照时，数据能被修改吗？
...
但是，如果主线程（父进程）要**修改共享数据里的某一块数据**（比如键值对 `A`
）时，就会发生写时复制，于是这块数据的**物理内存就会被复制一份（键值对 A'）**，然后

**主线程在这个数据副本（键值对**。与此同时，
`A'`
）进行修改操作**bgsave 子进程可以继续把原来的数据（键值对**。
`A`
）写入到 RDB 文件就是这样，Redis 使用 bgsave 对当前内存中的所有数据做快照，这个操作是由 bgsave 子进程在后台完成的，执行时不会阻塞主线程，这就使得主线程同时可以修改数据。
...
[#](https://www.xiaolincoding.com#rdb-和-aof-合体) RDB 和 AOF 合体
