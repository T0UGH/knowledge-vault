---
title: limit
source_url: https://www.xiaolincoding.com/mysql/index/limit.html
fetched_at: 2026-04-06
section: mysql
---

[#](https://www.xiaolincoding.com#mysql-分页有什么性能问题-怎么优化) MySQL 分页有什么性能问题？怎么优化？
大家好，我是小林。
...
[#](https://www.xiaolincoding.com#两种limit的执行过程) 两种limit的执行过程
...
[#](https://www.xiaolincoding.com#基于主键索引的limit执行过程) 基于主键索引的limit执行过程
那么回到文章开头的问题里。
...
, 这个操作，其实也是将在innodb中的主键索引中获取到`6000000+1`
...
[#](https://www.xiaolincoding.com#基于非主键索引的limit执行过程) 基于非主键索引的limit执行过程
...
```
通过`select id from page order by user_name limit 6000000, 100`
...
[#](https://www.xiaolincoding.com#深度分页问题) 深度分页问题
...
[#](https://www.xiaolincoding.com#如果你是想取出全表的数据) 如果你是想取出全表的数据
...
于是不少mysql小白会通过`limit offset size`
分页的形式去分批获取，刚开始都是好的，等慢慢地，哪天数据表变得奇大无比，就有可能出现前面提到的**深度分页**问题。
...
[#](https://www.xiaolincoding.com#如果是给用户做分页展示) 如果是给用户做分页展示
...
-
如果数据量很少，比如1k的量级，且长期不太可能有巨大的增长，还是用`limit offset, size`
的方案吧，整挺好，能用就行。
-
