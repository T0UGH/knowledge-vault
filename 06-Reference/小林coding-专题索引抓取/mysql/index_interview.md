---
title: index_interview
source_url: https://www.xiaolincoding.com/mysql/index/index_interview.html
fetched_at: 2026-04-06
section: mysql
---

[#](https://www.xiaolincoding.com#索引常见面试题) 索引常见面试题
大家好，我是小林。
...
如果你想针对MySQL进展专项的模拟面试，可以用小林和朋友合作打造的「[牛面AI面试 (opens new window)](https://www.niumianoffer.com/?ref=tswekk4d)」来检测MySQL的掌握情况：

难度是对标互联网中大厂的，问都是高频面试知识点，来一场满头大汗的模拟面试吧！

今天就带大家，夯实 MySQL 索引的知识点。

[#](https://www.xiaolincoding.com#什么是索引) 什么是索引？
...
[#](https://www.xiaolincoding.com#索引的分类) 索引的分类
...
[#](https://www.xiaolincoding.com#按数据结构分类) 按数据结构分类
...
[#](https://www.xiaolincoding.com#通过主键查询商品数据的过程) 通过主键查询商品数据的过程
比如，我们执行了下面这条查询语句：
...
[#](https://www.xiaolincoding.com#通过二级索引查询商品数据的过程) 通过二级索引查询商品数据的过程
...
[#](https://www.xiaolincoding.com#为什么-mysql-innodb-选择-b-tree-作为索引的数据结构) 为什么 MySQL InnoDB 选择 B+tree 作为索引的数据结构？
...
[#](https://www.xiaolincoding.com#按物理存储分类) 按物理存储分类
...
[#](https://www.xiaolincoding.com#按字段特性分类) 按字段特性分类
从字段特性的角度来看，索引分为主键索引、唯一索引、普通索引、前缀索引。

[#](https://www.xiaolincoding.com#主键索引) 主键索引
...
```
[#](https://www.xiaolincoding.com#唯一索引) 唯一索引
...
```
[#](https://www.xiaolincoding.com#普通索引) 普通索引
...
ON table_name(index_column_1,index_column_2,...);
```
[#](https://www.xiaolincoding.com#前缀索引) 前缀索引
...
```
[#](https://www.xiaolincoding.com#按字段个数分类) 按字段个数分类
...
建立在多列上的索引称为联合索引；
-
[#](https://www.xiaolincoding.com#联合索引) 联合索引
...
只有在 a 相同的情况才，b 才是有序的，比如 a 等于 2 的时候，b 的值为（7，8），这时就是有序的，这个有序状态是局部的，因此，执行`where a = 2 and b = 7`
是 a 和 b 字段能用到联合索引的，也就是联合索引生效了。

[#](https://www.xiaolincoding.com#联合索引范围查询) 联合索引范围查询
...
[#](https://www.xiaolincoding.com#索引下推) 索引下推
...
[#](https://www.xiaolincoding.com#索引区分度) 索引区分度
...
[#](https://www.xiaolincoding.com#联合索引进行排序) 联合索引进行排序
...
[#](https://www.xiaolincoding.com#什么时候需要-不需要创建索引) 什么时候需要 / 不需要创建索引？
...
[#](https://www.xiaolincoding.com#什么时候适用索引) 什么时候适用索引？
字段有唯一性限制的，比如商品编码；
-
经常用于 `WHERE`
...
-
[#](https://www.xiaolincoding.com#什么时候不需要创建索引) 什么时候不需要创建索引？
`WHERE`
条件，`GROUP BY`
，`ORDER BY`
...
-
[#](https://www.xiaolincoding.com#有什么优化索引的方法) 有什么优化索引的方法？
这里说一下几种常见优化索引的方法：
...
防止索引失效；
-
[#](https://www.xiaolincoding.com#前缀索引优化) 前缀索引优化
...
无法把前缀索引用作覆盖索引；
-
[#](https://www.xiaolincoding.com#覆盖索引优化) 覆盖索引优化
...
[#](https://www.xiaolincoding.com#主键索引最好是自增的) 主键索引最好是自增的
...
[#](https://www.xiaolincoding.com#索引最好设置为-not-null) 索引最好设置为 NOT NULL
为了更好的利用索引，索引列要设置为 NOT NULL 约束。有两个原因：

第一原因：索引列存在 NULL 就会导致优化器在做索引选择的时候更加复杂，更加难以优化，因为可为 NULL 的列会使索引、索引统计和值比较都更复杂，比如进行索引统计时，count 会省略值为NULL 的行。

第二个原因：NULL 值是一个没意义的值，但是它会占用物理空间，所以会带来的存储空间的问题，因为 InnoDB 存储记录的时候，如果表中存在允许为 NULL 的字段，那么

[行格式 (opens new window)](https://xiaolincoding.com/mysql/base/row_format.html#innodb-%E8%A1%8C%E6%A0%BC%E5%BC%8F%E6%9C%89%E5%93%AA%E4%BA%9B)中**至少会用 1 字节空间存储 NULL 值列表**，如下图的紫色部分：
[#](https://www.xiaolincoding.com#防止索引失效) 防止索引失效
