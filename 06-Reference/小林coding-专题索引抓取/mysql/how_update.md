---
title: how_update
source_url: https://www.xiaolincoding.com/mysql/log/how_update.html
fetched_at: 2026-04-06
section: mysql
---

[#](https://www.xiaolincoding.com#mysql-日志-undo-log、redo-log、binlog-有什么用) MySQL 日志：undo log、redo log、binlog 有什么用？
大家好，我是小林。
...
[#](https://www.xiaolincoding.com#为什么需要-undo-log) 为什么需要 undo log？
...
[#](https://www.xiaolincoding.com#为什么需要-buffer-pool) 为什么需要 Buffer Pool？
...
-
[#](https://www.xiaolincoding.com#buffer-pool-缓存什么) Buffer Pool 缓存什么？
InnoDB 会把存储的数据划分为若干个「页」，以页作为磁盘和内存交互的基本单位，一个页的默认大小为 16KB。因此，Buffer Pool 同样需要按「页」来划分。

在 MySQL 启动的时候，**InnoDB 会为 Buffer Pool 申请一片连续的内存空间，然后按照默认的 16KB的大小划分出一个个的页， Buffer Pool 中的页就叫做缓存页**。此时这些缓存页都是空闲的，之后随着程序的运行，才会有磁盘上的页被缓存到 Buffer Pool 中。
...
[#](https://www.xiaolincoding.com#为什么需要-redo-log) 为什么需要 redo log ？
...
[#](https://www.xiaolincoding.com#redo-log-什么时候刷盘) redo log 什么时候刷盘？
...
针对参数 0 ：会把缓存在 redo log buffer 中的 redo log ，通过调用 `write()`
...
在一些对数据安全性要求比较高的场景中，显然 `innodb_flush_log_at_trx_commit`
参数需要设置为 1。
-
...
-
[#](https://www.xiaolincoding.com#redo-log-文件写满了怎么办) redo log 文件写满了怎么办？
...
[#](https://www.xiaolincoding.com#为什么需要-binlog) 为什么需要 binlog ？
...
[#](https://www.xiaolincoding.com#redo-log-和-binlog-有什么区别) redo log 和 binlog 有什么区别？
这两个日志有四个区别。
...
[#](https://www.xiaolincoding.com#主从复制是怎么实现) 主从复制是怎么实现？
...
[#](https://www.xiaolincoding.com#binlog-什么时候刷盘) binlog 什么时候刷盘？
...
-
[#](https://www.xiaolincoding.com#为什么需要两阶段提交) 为什么需要两阶段提交？
...
[#](https://www.xiaolincoding.com#两阶段提交的过程是怎样的) 两阶段提交的过程是怎样的？
...
[#](https://www.xiaolincoding.com#异常重启会出现什么现象) 异常重启会出现什么现象？
...
[#](https://www.xiaolincoding.com#两阶段提交有什么问题) 两阶段提交有什么问题？
...
[#](https://www.xiaolincoding.com#组提交) 组提交
...
绿色这一组事务的 binlog 写入到 binlog 文件后，并不会马上执行刷盘的操作，而是**会等待一段时间**，这个等待的时长由 `Binlog_group_commit_sync_delay`
...
[#](https://www.xiaolincoding.com#mysql-磁盘-i-o-很高-有什么优化的方法) MySQL 磁盘 I/O 很高，有什么优化的方法？
