---
title: -h 指定 MySQL 服务得 IP 地址，如果是连接本地的 MySQL服务，可以不用这个参数；
source_url: https://www.xiaolincoding.com/mysql/base/how_select.html
fetched_at: 2026-04-06
section: mysql
---

[#](https://www.xiaolincoding.com#执行一条-select-语句-期间发生了什么) 执行一条 select 语句，期间发生了什么？
大家好，我是小林。
...
[#](https://www.xiaolincoding.com#mysql-执行流程是怎样的) MySQL 执行流程是怎样的？
...
[#](https://www.xiaolincoding.com#第一步-连接器) 第一步：连接器
...
-
[#](https://www.xiaolincoding.com#第二步-查询缓存) 第二步：查询缓存
...
[#](https://www.xiaolincoding.com#第三步-解析-sql) 第三步：解析 SQL
在正式执行 SQL 查询语句之前， MySQL 会先对 SQL 语句做解析，这个工作交由「解析器」来完成。

[#](https://www.xiaolincoding.com#解析器) 解析器
解析器会做如下两件事情。
...
[#](https://www.xiaolincoding.com#第四步-执行-sql) 第四步：执行 SQL
经过解析器后，接着就要进入执行 SQL 查询语句的流程了，每条`SELECT`
查询语句流程主要可以分为下面这三个阶段：
...
execute 阶段，也就是执行阶段；
-
[#](https://www.xiaolincoding.com#预处理器) 预处理器
我们先来说说预处理阶段做了什么事情。
...
[#](https://www.xiaolincoding.com#优化器) 优化器
...
[#](https://www.xiaolincoding.com#执行器) 执行器
...
索引下推
-
[#](https://www.xiaolincoding.com#主键索引查询) 主键索引查询
...
执行器第一次查询，会调用 read_first_record 函数指针指向的函数，因为优化器选择的访问类型为 const，这个函数指针被指向为 InnoDB 引擎索引查询的接口，把条件 `id = 1`
...
[#](https://www.xiaolincoding.com#全表扫描) 全表扫描
举个全表扫描的例子：
