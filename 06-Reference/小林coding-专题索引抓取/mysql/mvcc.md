---
title: mvcc
source_url: https://www.xiaolincoding.com/mysql/transaction/mvcc.html
fetched_at: 2026-04-06
section: mysql
---

[#](https://www.xiaolincoding.com#事务隔离级别是怎么实现的) 事务隔离级别是怎么实现的？
这是我的钱包，共有 100 万元。
...
[#](https://www.xiaolincoding.com#事务有哪些特性) 事务有哪些特性？
...
[#](https://www.xiaolincoding.com#并行事务会引发什么问题) 并行事务会引发什么问题？
...
[#](https://www.xiaolincoding.com#脏读) 脏读
...
[#](https://www.xiaolincoding.com#不可重复读) 不可重复读
**在一个事务内多次读取同一个数据，如果出现前后两次读到的数据不一样的情况，就意味着发生了「不可重复读」现象。**
举个栗子。

假设有 A 和 B 这两个事务同时在处理，事务 A 先开始从数据库中读取小林的余额数据，然后继续执行代码逻辑处理，**在这过程中如果事务 B 更新了这条数据，并提交了事务，那么当事务 A 再次读取该数据时，就会发现前后两次读到的数据是不一致的，这种现象就被称为不可重复读。**

[#](https://www.xiaolincoding.com#幻读) 幻读
...
[#](https://www.xiaolincoding.com#事务的隔离级别有哪些) 事务的隔离级别有哪些？
...
**读未提交（**，指一个事务还没提交时，它做的变更就能被其他事务看到；*read uncommitted*）
**读提交（**，指一个事务提交之后，它做的变更才能被其他事务看到；*read committed*）
**可重复读（**，指一个事务执行过程中看到的数据，一直跟这个事务启动时看到的数据是一致的，*repeatable read*）**MySQL InnoDB 引擎的默认隔离级别**；
**串行化（**；会对记录加上读写锁，在多个事务对这条记录进行读写操作时，如果发生了读写冲突的时候，后访问的事务必须等前一个事务执行完成，才能继续执行；*serializable*）
按隔离水平高低排序如下：
...
[#](https://www.xiaolincoding.com#read-view-在-mvcc-里如何工作的) Read View 在 MVCC 里如何工作的？
我们需要了解两个知识：
...
值，表示这个版本的记录是在创建 Read View**前**已经提交的事务生成的，所以该版本的记录对当前事务**可见**。
-
如果记录的 trx_id 值大于等于 Read View 中的 `max_trx_id`
值，表示这个版本的记录是在创建 Read View**后**才启动的事务生成的，所以该版本的记录对当前事务**不可见**。
-
如果记录的 trx_id 值在 Read View 的 `min_trx_id`
和`max_trx_id`
之间，需要判断 trx_id 是否在 m_ids 列表中：如果记录的 trx_id **在**`m_ids`
列表中，表示生成该版本记录的活跃事务依然活跃着（还没提交事务），所以该版本的记录对当前事务**不可见**。
-
...
[#](https://www.xiaolincoding.com#可重复读是如何工作的) 可重复读是如何工作的？
...
[#](https://www.xiaolincoding.com#读提交是如何工作的) 读提交是如何工作的？
