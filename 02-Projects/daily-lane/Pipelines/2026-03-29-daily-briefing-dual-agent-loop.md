...
- **Subagent B / Reviewer**
  ...
  - 结合 Editor 草稿做审核
  - 必要时自己回查原始 signal 文件
  - 检查漏项、重复、证据不足、推荐理由是否成立
  - 给出 pass / revise 判断

- **一个 cron**
  - 负责每天定时触发
  - 只收集并喂入各 lane `index.md`
  - 调 Editor
  - 调 Reviewer
...
截至当前，这个日报内容层第一版还进一步拍定了几条关键约束：

### 1. 第一版先只做完整 markdown 日报
...
这是因为当前底层本来就是 lane-first，第一版先不额外增加跨 lane 压缩层，更稳，也更容易审核。
...
- 可以一条 signal 对应一条日报项
- 也可以由 Editor 合并多个相关 signal

但不能为了变短而损失信息。

更准确地说：

> **第一版日报默认偏完整、偏详细、偏可追溯，而不是偏短。**
